...
- **Item list, per role** (carrier-catalog ticket `agent-maintains-carriers`): Agent Console — Dashboard, Requests, Fleet, Carriers, Client Invoices, My Invoice; Manager Console — Dashboard, Clients, Agents, Contracts, Carriers, Invoices; Client Portal unchanged (no Carriers access).
