...
import com.remotesupport.backend.security.ContractAccessGuard;
...
public class ContractController {
  ...
  /**
   * Manager sees every Contract in the tenant; an Agent only their own; a Tester only their
   * Client's — the same scoping Fleet visibility uses, needed here so a Contract switcher has
   * something to switch between (spec.md Access control: "every actor sees only their own
   * Contracts..."). Scoping itself lives in {@link ContractAccessGuard}, the same convention
   * {@link com.remotesupport.backend.security.FleetAccessGuard} and its siblings established.
   */
  @GetMapping
  public List<ContractResponse> list(@AuthenticationPrincipal AuthenticatedPrincipal principal) {
    return contractAccessGuard.visibleContracts(principal).stream().map(ContractResponse::of).toList();
  }
}
