...
- [ ] Replace Smartphone and Replace SIM are Request types and Fee types, offered to Testers and Agents
- [ ] A Replace Smartphone Request requires an Active Smartphone of its Contract and may name a different model; a Replace SIM Request requires an Active SIM Card of its Contract
- [ ] Completing a Replace Smartphone retires the named Smartphone, adds a company-owned one with the requested or the same model, and moves the old one's SIM Cards into it; no Agent input
- [ ] Completing a Replace SIM asks for the new SIM Card's details, defaulted from the old one's; the old SIM Card is retired and the new one is installed where the old one was
- [ ] Completion is refused with a clear message if the named unit is no longer Active
- [ ] A Fee can be logged against a Replace Request
- [ ] The Requests lists name the unit being replaced
