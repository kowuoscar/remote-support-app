...
/**
 * A Tester's read-only view of a Contract's current-month Client Invoice
 * (client-invoice-submission-and-visibility ticket AC: "Once sent, every Tester at that
 * Contract's Client can view the Client Invoice read-only" / "A draft Client Invoice is never
 * visible to a Tester"; user stories 34-35). Mirrors AgentClientInvoicesView's Contract-switcher
 * + summary-card shape, minus every write affordance (no attach control, no Send/Approve) plus
 * the one action a Tester does have: generating a PDF on demand. A `null` entry — no invoice this
 * Contract has sent yet, or one that's still `draft` — renders the same empty state either way,
 * since the backend itself makes the two indistinguishable to a Tester (ClientInvoiceAccessGuard).
 */
  ...
  if (contracts.length === 0) {
    return (
      <EmptyState
        ...
        description="Once your Agent sends a Client Invoice for one of your Contracts, it shows up here."
      />
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {!invoice ? (
        <EmptyState
          icon={<IconInvoices className="h-5 w-5" />}
          title="No Invoice sent yet for this Contract"
          description="Your Agent hasn't sent a Client Invoice for this Contract this month yet. It will appear here as soon as they do."
        />
      ) : (
