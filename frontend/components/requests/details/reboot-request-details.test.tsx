...
describe("RebootRequestDetails", () => {
  it("offers only Active Smartphones, and requires a choice", () => {
    ...
    const picker = screen.getByRole("combobox", { name: "Smartphone to reboot" });
  ...
  it("explains there is nothing to pick when no Smartphone is Active", () => {
    render(<RebootRequestDetails smartphones={[]} simCards={[]} carriers={[]} carriersHref="" currency="USD" />);

    expect(screen.queryByRole("combobox", { name: "Smartphone to reboot" })).not.toBeInTheDocument();
