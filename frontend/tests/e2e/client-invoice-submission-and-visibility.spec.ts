 ...
 * downloads a PDF, and a Manager approves it — driven against a real backend + Postgres (see
...
test.describe("client invoice submission and visibility", () => {
  test("an agent sends a client invoice, a tester views and downloads it, a manager approves it", async ({
    ...
    // Manager reviews and approves it from the Contract detail page.
    await login(page, SEEDED_USERS.manager.username, SEEDED_USERS.manager.password);
    await page.goto(`/manager/contracts/${contractId}`);
    await expect(page.getByText("Awaiting approval")).toBeVisible();
    ...
    const [approveResponse] = await Promise.all([
      page.waitForResponse(
        (resp) => resp.url().includes("/client-invoice/approve") && resp.request().method() === "POST",
      ),
